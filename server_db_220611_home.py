...
class ServerDB:
    def __init__(self):
        ...
        # 서버 DB 스레드
        # th_id_pw_db_check = Thread(target=self.receive_id_pw_db_check)
    ...
    def receive_msg_process(self, c_socket):
        while True:
            ...
            # elif "$tid$" in self.receive_id_pw:
                ...
                self.send_english_word_process("$learn_eng$")
                break
            elif "$quiz_eng$" in self.receive_msg:
                print("$quiz_eng$ 메시지 받음")
                self.send_english_word_process("$quiz_eng$")
                break
            elif "$answer$" in self.receive_msg:
                print("채점 결과 DB 저장 확인용")
    ...
    def send_english_word_process(self, recv_msg):
