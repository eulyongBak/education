...
form_class = uic.loadUiType("teacher_client_ui.ui")[0]

class UiInputInformation(QMainWindow, form_class):
    def __init__(self, ip, port):
        super().__init__()
        # self.teacher_client_socket = None
        self.setupUi(self)
        ...
        # 조사(1페이지) 시작 버튼
        # self.btn_teacher_login.clicked.connect(self.move_next_page)
        self.btn_teacher_login.clicked.connect(self.login_progress)
        # self.btn_teacher_learning.clicked.connect(self.move_next_page)
        # self.btn_teacher_quiz.clicked.connect(self.move_next_page2)
        # self.btn_teacher_consulting.clicked.connect(self.move_next_page3)
        # self.btn_learning_main.clicked.connect(self.move_before_page2)


        # 조사(1페이지) 종료 버튼
        self.btn_teacher_exit.clicked.connect(quit)

    def initialize_socket(self, sock_ip, sock_port):
        '''
        TCP socket을 생성하고 server와 연결
        '''
        self.teacher_client_socket = socket(AF_INET, SOCK_STREAM)
        remote_ip = sock_ip
        remote_port = sock_port
        print("remote_ip = ", remote_ip)
        print("remote_port = ", remote_port)
        self.teacher_client_socket.connect((remote_ip, remote_port))

    # def move_next_page(self):
    def login_progress(self):
        self.teacher_id = "@tid@" + self.te_teacher_id.toPlainText()
        self.teacher_pw = "@tpw@" + self.te_teacher_pw.toPlainText()
        self.teacher_id_pw = self.teacher_id + "/" + self.teacher_pw
        print("ID: ", self.teacher_id)
        print("PW: ", self.teacher_pw)
        # self.teacher_client_socket.send(self.teacher_id.encode())
        # self.teacher_client_socket.send(self.teacher_pw.encode())
        self.teacher_client_socket.send(self.teacher_id_pw.encode())
    ...
    def listen_thread(self):
        ...
        login_ok_thread = Thread(target=self.move_main_page, args=(self.teacher_client_socket,))
